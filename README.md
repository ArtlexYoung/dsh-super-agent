...
未显式设置 `planning` 时，工作流会根据 profile 选择默认值：`solo × delivery` 保持自适应低成本路径；`team × delivery`、`research` 和 `optimization` 先生成结构化分析，以便记录任务拆分、证据范围或 baseline。显式的 `planning` 选项始终优先。
