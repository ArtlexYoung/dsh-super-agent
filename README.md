...
- 当前包版本：`0.0.2`。
- 目标 Harness：`0.1.2-alpha.2` 或更新的 0.1.2 发行线；当前包对 `@deepseek-ai/dsh-agent-presets` 的依赖从 `0.1.2-alpha.2` 开始，npm lock 当前解析为 `0.1.2-rc.1`。pinned `cd5ef814`（runtime `0.1.2-alpha.1`）上的四个 preset 解析和 `deepseek/deepseek-flash` smoke 属于 0.0.1 历史基线，不作为当前包的完整兼容证明。
...
详细维护计划、架构记录和本地测试日志位于被忽略的 `.development/`，不会进入 Git 或 npm 包。市场目录登记仍与 npm 发布分开处理。
